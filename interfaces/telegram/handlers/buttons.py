...
from aiogram.types import Message, ReactionTypeEmoji, ReplyParameters, ReplyKeyboardRemove
...
async def button_hide(message: Message):
    """Кнопка скрытия клавиатуры."""
    logger.info(f"Кнопка 'Скрыть' от пользователя {message.from_user.id}")

    await message.answer(
        "Клавиатура скрыта. Напишите /menu, чтобы показать её снова.",
...
def register(router: Router) -> None:
    ...
    router.message.register(button_hide, F.text == "🔽 Скрыть")